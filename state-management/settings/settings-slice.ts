...
// import { FeaturesService , AccountSettingsService , ContactSettingService SettingsService EmailSettingsService} from "../../proxy";
...
export const postEmailingSendTestEmail = createAsyncThunk(
    "settings/postEmailingSendTestEmail",
    (formData:any)=>{
        return EmailSettingsService?.postEmailingSendTestEmail({requestBody:{senderEmailAddress: "Test@gmail.com",
            targetEmailAddress: formData,
            subject: "Testing email functionality",
            body: "I hope this email finds you well."+

           "We are conducting a test to check the functionality of our email system. This is a routine check to ensure that our emails are being delivered promptly and that all links and attachments are working as expected."+
            
            "Thank you."}}).then((result:any)=>{
            return result 
        })
    })
...
const settingsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(saveContactSettings.pending, (state) => {
            state.loading = true;
        });
        builder.addCase(
            saveContactSettings.fulfilled,
            (state) => {
                state.loading = false;
            }
        );
        builder.addCase(saveContactSettings.rejected, (state, action) => {
            state.loading = false;
            state.error = action.error.message || "Something went wrong";
        });
    },
